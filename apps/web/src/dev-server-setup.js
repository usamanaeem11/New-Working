...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all Working Tracker.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*Working Tracker\.sh$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@Working Tracker.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@Working Tracker.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
